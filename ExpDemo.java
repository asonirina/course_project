...
public class ExpDemo {
    ...
    public void printTree1(CommonTree t, int indent) {
     //   System.out.println(t.toString());
        ...
        nodes.add(root);
        printTreeHelper1(t, indent, root);
    }

    private void printTreeHelper1(CommonTree t, int indent, TreeNode node) {
        if (t != null) {
            StringBuffer sb = new StringBuffer(indent);
            for (int i = 0; i <indent; i++)
                sb = sb.append("___");
            for (int i = 0; i < t.getChildCount(); i++) {
                ...
                TreeNode child = new TreeNode(t.getChild(i).getType()+" "+t.getChild(i).getText(), node);
                nodes.add(child);
                printTreeHelper1((CommonTree) t.getChild(i), indent + 1, child);
            }
        }
    }
    public static void main(String[] args) throws Exception {
        ...
        ANTLRInputStream in = new ANTLRInputStream(new FileInputStream("Test.java"));
